...
%>				
				...
				<tr>
				<TD> <%= rs.getString("test_name") %> </TD><td><a href="test_record_view.jsp?srs=<%=reqid%>&username=<%=username%>">View Details</a></td>
				</tr>
				</table>
<%			}
			}
...
try{
			Class.forName("com.mysql.jdbc.Driver"); 
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost/test","root","root123"); 
			String qry = "update test_record set status='dev_test_submit' where req_no='"+reqid+"'";
			//out.println("::"+qry);
			Statement st=con.createStatement();
...
<%try{
			Class.forName("com.mysql.jdbc.Driver"); 
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost/test","root","root123"); 
			String qry = "select * from test_record where devid='"+username+"' and status='dev_test_submit'";
			//out.println("::"+qry);
			Statement st=con.createStatement();
