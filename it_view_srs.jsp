...
<%		String devid=request.getParameter("select1");
		try{
			...
			String qry = "select * from srs where devid='"+devid+"' and status is null";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(qry);
			while(rs.next())				
			{	String path=rs.getString("srs_name");	
...
%>				<center>
				...
				<!--a href="srs_view.jsp?srs=<!%=req_no%>">View Details</a-->
				<form action="it_srs_approve.jsp" method="post">
				<input type="hidden" name="username" value="<%=devid%>">
				<input type="hidden" name="srs" value="<%=req_no%>">
				<%session.setAttribute("username",devid);%>
				<input type="submit" value="Accept">
				</form>
				<form action="it_srs_reject.jsp" method="post">
				<input type="hidden" name="username" value="<%=devid%>">
				<input type="hidden" name="srs" value="<%=req_no%>">
				<%session.setAttribute("username",devid);%>
...
<!--a href="it_srs_submit.jsp?username=<!%=devid%>">submited List</a> <br><br>
<a href="it_srs_reject.jsp?username=<!%=devid%>">Rejected List</a-->
				<br><br><center><form action="it_srs_reject.jsp" method="post">
				<input type="hidden" name="username" value="<%=devid%>">
				<input type="submit" value="Rejected List">
				</form></center>
				<center>
				<form action="it_srs_submit.jsp" method="post">
				<input type="hidden" name="username" value="<%=devid%>">
