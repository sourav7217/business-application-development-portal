...
<%		String devid=request.getParameter("select1");
		try{
			...
			String qry = "select * from test_record where devid='"+devid+"' and status='dept_approve'";
...
%>				<center>
				...
				<form action="it_test_record_approve.jsp?srs=<%=req_no%>&username=<%=devid%>" method="post">
				<input type="hidden" name="select1" value="<%=devid%>">
				<%session.setAttribute("username",devid);%>
				<input type="submit" value="Accept">
				</form>
				<form action="it_test_record_reject.jsp?srs=<%=req_no%>&username=<%=devid%>" method="post">
				<%session.setAttribute("username",devid);%>
...
%>
<center><br><br><a href="it_test_record_submit.jsp?username=<%=devid%>">submited List</a> <br><br>
<a href="it_test_record_reject.jsp?username=<%=devid%>">Rejected List</a><center>
</html>
</body>