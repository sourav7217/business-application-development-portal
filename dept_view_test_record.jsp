...
<%		String devid=request.getParameter("select1");
		...
		try{
			...
			String qry = "select * from test_record where devid='"+devid+"' and status='client_submit' and department=(select department from dept_hod where userid='"+username+"')";
			//out.print(""+qry);
...
%>				<center>
				...
				<strong><%=path%></strong>
				<a href="test_record_view.jsp?srs=<%=req_no%>">View Details</a>
				<form action="dept_test_record_approve.jsp?srs=<%=req_no%>&username=<%=devid%>" method="post">
				<input type="hidden" name="select1" value="<%=devid%>">
				<%session.setAttribute("username",devid);%>
				<input type="submit" value="Accept">
				</form>
				<form action="dept_test_record_reject.jsp?srs=<%=req_no%>&username=<%=devid%>" method="post">
				<%session.setAttribute("username",devid);%>
